...
package com.example.schoolquery.mutation.compiler;
...
public final class MutationCompiler {
    ...
    private MutationPlan compileUpdate(Update update, SysModule explicit) {
        ...
        if (update.getUpdateSets() != null) {
            for (net.sf.jsqlparser.statement.update.UpdateSet set : update.getUpdateSets()) {
                for (int i = 0; i < set.getColumns().size(); i++) {
                    assignments.add(new MutationPlan.Assignment(resolveWritable(module.id(), set.getColumns().get(i).getColumnName()), literal(set.getValues().get(i).toString())));
                }
            }
