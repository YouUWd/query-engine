...
public final class MutationCompiler {
    ...
    private MutationPlan compileInsert(Insert insert) {
        ...
        String valuesText = select.toString();
        ...
        for (int i = 0; i < insert.getColumns().size(); i++)
            a.add(new MutationPlan.Assignment(resolve(module.id(), insert.getColumns().get(i).getColumnName()), literal(values.get(i))));
    ...
    private MutationPlan.Where where(long moduleId, Expression e) {
        ...
        if (!(e instanceof EqualsTo x)) throw new IllegalArgumentException("DML WHERE currently supports only equality predicates");
        return new MutationPlan.Where(List.of(new MutationPlan.Predicate(
                resolve(moduleId, x.getLeftExpression().toString()), "EQ", literal(x.getRightExpression().toString()))));
    }
    ...
    private LogicalFieldRef resolve(long moduleId, String raw) {
        ...
        List<SysModuleField> m = new ArrayList<>();
        for (List<SysModuleField> fs : registry.fieldsGroupedByTable(moduleId).values())
            for (SysModuleField f : fs) if (f.columnName().equals(column)) m.add(f);
