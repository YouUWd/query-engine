...
/**
 * Builds the physical query tree from logical modules. Virtual modules are UI/
 * grouping concepts and are intentionally transparent to this tree.
 */
public class QueryTreeBuilder {
    ...
    public FlatGroup buildFromRoot(long rootModuleId, Set<Long> touchedModuleIds) {
        ...
        return buildGroup(rootModuleId, registry.module(rootModuleId), backbone);
    }
    ...
    public FlatGroup buildAutoRoot(Set<Long> touchedModuleIds) {
        ...
        return buildGroup(root, registry.module(root), backbone);
    }
    ...
    private void processChildren(SysModule anchor, SysModule current, Set<Long> backbone,
        ...
        for (SysModule child : registry.children(current.id())) {
            ...
            if (child.isVirtual()) {
                // Transparent: preserve the physical anchor and continue traversal.
                processChildren(anchor, child, backbone, merged, nested);
                continue;
            }
