...
public final class PlanConditionCompiler {
    ...
    public PlanConditionCompiler(MetadataRegistry registry) {
    ...
    private Condition compileExpression(DSLContext dsl, long rootId, FilterExpressionPlan expression) {
        ...
        if (expression instanceof FilterExpressionPlan.Or o) {
            Condition result = falseCondition();
            for (FilterExpressionPlan child : o.children()) result = result.or(compileExpression(dsl, rootId, child));
            return result;
        }
        throw new IllegalArgumentException("Unsupported filter expression: " + expression);
    }

    private Condition compilePredicate(DSLContext dsl, long rootId, FilterPlan filter) {
    ...
    private Condition descendantExists(DSLContext dsl, SysModule root, SysModule owner,
                                       SysModuleField fieldMeta, FilterPlan filter) {
        List<SysModule> path = modulePath(root, owner);
        if (path.size() < 2) return predicateCondition(field(fieldMeta), filter);

        SysModule leaf = path.get(path.size() - 1);
        ...
        for (int i = path.size() - 2; i >= 0; i--) {
            ...
            if (relation.parentColumn().isBlank() || relation.childColumn().isBlank()) {
                throw new IllegalArgumentException("No physical key mapping for module relation " + parent.id() + " -> " + child.id());
            }
            Table<?> parentTable = table(name(parent.primaryTable()));
            Table<?> childTable = table(name(child.primaryTable()));
            Field<Object> parentKey = field(name(parentTable.getName(), relation.parentColumn()), Object.class);
            Field<Object> childKey = field(name(childTable.getName(), relation.childColumn()), Object.class);
            inner = inner.and(parentKey.eq(childKey));
            inner = exists(dsl.selectOne().from(childTable).where(inner));
            child = parent;
        }
        return inner;
    }

    private List<SysModule> modulePath(SysModule root, SysModule owner) {
        List<Long> chain = registry.ancestorChain(owner.id());
    ...
    private boolean samePhysicalEntity(SysModule root, SysModule owner) {
        return root.primaryTable().equals(owner.primaryTable()) && registry.ancestorChain(owner.id()).contains(root.id());
    }
