...
/** Renders a QueryTree. Nested relations are already resolved by logical module identity. */
public class FlatGroupSqlBuilder {
    ...
    public SelectConditionStep<Record> build(DSLContext dsl, FlatGroup group, Map<Long,List<SysModuleField>> requestedByModule, Condition condition) { return build(dsl,group,requestedByModule,condition,null,Map.of()); }
    public SelectConditionStep<Record> build(DSLContext dsl, FlatGroup group, Map<Long,List<SysModuleField>> requestedByModule, Condition condition, PermissionContext ctx) { return build(dsl,group,requestedByModule,condition,ctx,Map.of()); }
    public SelectConditionStep<Record> build(DSLContext dsl, FlatGroup group, Map<Long,List<SysModuleField>> requestedByModule, Condition condition, PermissionContext ctx, Map<String,Condition> extra) {
        ...
        for(NestedGroup nested:group.nestedChildren()) { ResolvedRelationPlan rel=nested.relation(); Field<Object> parentKey=DynamicFields.field(primary,rel.parentColumn()); Table<?> child=table(name(nested.group().primaryTable())); Field<Object> childFk=DynamicFields.field(child,rel.childColumn()); SelectConditionStep<Record> childSelect=build(dsl,nested.group(),requestedByModule,childFk.eq(parentKey),ctx,extra); fields.add(multiset(childSelect).as(nestedAlias(nested.childModuleId()))); }
    ...
    public SelectConditionStep<Record1<Integer>> buildCountQuery(DSLContext dsl, FlatGroup root, Map<Long,List<SysModuleField>> requestedByModule, Condition condition, PermissionContext ctx, Map<String,Condition> extra) {
        Table<?> primary=table(name(root.primaryTable())); Map<String,Table<?>> joins=new LinkedHashMap<>(); Map<String,Condition> joinConditions=new LinkedHashMap<>();
        for(long mid:root.mergedModuleIds()) for(SysModuleField f:requestedByModule.getOrDefault(mid,List.of())) if(!f.tableName().equals(root.primaryTable())) { Table<?> owner=joins.computeIfAbsent(f.tableName(),t->table(name(t))); joinConditions.computeIfAbsent(f.tableName(),t->buildFlatJoinCondition(primary,root.primaryTable(),owner,t)); }
        SelectJoinStep<Record1<Integer>> from=dsl.selectCount().from(primary); SelectOnConditionStep<Record1<Integer>> joined=null; for(Map.Entry<String,Table<?>> e:joins.entrySet()) { SelectJoinStep<Record1<Integer>> base=joined==null?from:joined; joined=base.leftJoin(e.getValue()).on(joinConditions.get(e.getKey())); }
        Condition scoped=applyDataScope(primary,root.primaryTable(),condition,ctx); Condition withExtra=applyExtraCondition(root.primaryTable(),scoped,extra); return joined!=null?joined.where(withExtra):from.where(withExtra);
    }
    ...
    /** Legacy physical-table join API retained only for compatibility. */
