...
package com.example.schoolquery.query.executor;
...
import com.example.schoolquery.query.model.FilterExpressionPlan;
import com.example.schoolquery.query.model.LogicalFieldRef;
import com.example.schoolquery.query.renderer.PlanConditionCompiler;
...
public final class QueryPlanExecutor {
    ...
    private List<SortField<?>> buildOrderBy(QueryPlan plan, FlatGroup tree) {
        ...
        for (var item : plan.sort().items()) {
            ...
            result.add(item.direction() == com.example.schoolquery.query.model.SortPlan.Direction.DESC ? column.desc() : column.asc());
        }
        return result;
    }
    ...
    private List<ColumnMeta> metadata(QueryPlan plan) {
        ...
        for (int i = 0; i < plan.projections().size(); i++) {
            ...
            result.add(new ColumnMeta(field.id(), module.id(), module.moduleName(), field.tableName(), field.columnName(),
                    plan.projectionAliases().get(i), com.example.schoolquery.metadata.SysFieldType.UNKNOWN));
        }
        return result;
    }
}
