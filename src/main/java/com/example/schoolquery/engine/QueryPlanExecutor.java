...
public final class QueryPlanExecutor {
    ...
    public ModuleQueryResult execute(DSLContext dsl, QueryPlan plan) {
        ...
        List<SortField<?>> orderBy=buildOrderBy(plan);
        SelectLimitStep<Record> limited=orderBy.isEmpty()?select:select.orderBy(orderBy);
        Result<Record> rows=plan.pagination()==null?limited.fetch():limited.limit(plan.pagination().pageSize()).offset(plan.pagination().offset()).fetch();
    ...
    private Map<Long,List<SysModuleField>> resolveProjection(List<LogicalFieldRef> projections) {
        ...
        for(LogicalFieldRef ref:projections){
            ...
            if(field.moduleId()!=ref.moduleId()) throw new IllegalArgumentException("fieldId="+ref.fieldId()+" does not belong to moduleId="+ref.moduleId());
    ...
    private List<ColumnMeta> metadata(List<LogicalFieldRef> projections){
        ...
        for(LogicalFieldRef ref:projections){
            SysModuleField field=registry.field(ref.fieldId()); var module=registry.module(ref.moduleId());
            result.add(new ColumnMeta(field.id(),module.id(),module.moduleName(),field.tableName(),field.columnName(),FlatGroupSqlBuilder.fieldAlias(field.id()),com.example.schoolquery.model.SysFieldType.UNKNOWN));
        }
        return result;
    }
}
