...
public final class QueryPlanExecutor {
    ...
    public ModuleQueryResult execute(DSLContext dsl, QueryPlan plan) {
        ...
        if (plan.projections().isEmpty()) {
            throw new IllegalArgumentException("SELECT projection must not be empty");
        }
        ...
        List<Map<String, Object>> rendered = rows.stream()
                .map(r -> renderer.renderRecord(plan.rootModuleId(), tree, r, requested))
                .toList();
        return new ModuleQueryResult(metadata(plan.projections()), rendered);
    }

    private Map<Long, Condition> buildLocalConditions(
            DSLContext dsl, long root, FilterExpressionPlan expression) {
        if (expression == null) return Map.of();
        Map<Long, Condition> result = new LinkedHashMap<>();
        for (var module : registry.allModules()) {
            if (module.id() == root
                    || !registry.ancestorChain(module.id()).contains(root)
                    || module.isVirtual()) continue;
        ...
        for (LogicalFieldRef ref : projections) {
            ...
            if (field.moduleId() != ref.moduleId()) {
                throw new IllegalArgumentException(
                        "fieldId=" + ref.fieldId() + " does not belong to moduleId=" + ref.moduleId());
            }
    ...
    private List<SortField<?>> buildOrderBy(QueryPlan plan, FlatGroup tree) {
        ...
        for (var item : plan.sort().items()) {
            ...
            result.add(item.direction() == com.example.schoolquery.plan.SortPlan.Direction.DESC
    ...
    private Table<?> ownerTable(FlatGroup tree, String tableName) {
        if (tree.primaryTable().equals(tableName)) return table(name(tableName));
        if (containsTable(tree, tableName)) return table(name(tableName));
    ...
    private boolean containsTable(FlatGroup group, String tableName) {
        for (long moduleId : group.mergedModuleIds()) {
            for (SysModuleField field : registry.fieldsGroupedByTable(moduleId)
                    .getOrDefault(tableName, List.of())) return true;
        }
    ...
    private List<ColumnMeta> metadata(List<LogicalFieldRef> projections) {
        ...
        for (LogicalFieldRef ref : projections) {
            ...
            result.add(new ColumnMeta(field.id(), module.id(), module.moduleName(), field.tableName(),
                    field.columnName(), FlatGroupSqlBuilder.fieldAlias(field.id()),
