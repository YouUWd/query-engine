...
public final class ModuleQueryCompiler {
    ...
    public QueryPlan compile(String sql) {
        ...
        for(ModuleSqlAst.SortSpec sort:ast.sorts()) sorts.add(new SortPlan.SortItem(resolve(root.id(),sort.expression()),sort.ascending()?SortPlan.Direction.ASC:SortPlan.Direction.DESC));
        PaginationPlan page=ast.limit()==null?null:new PaginationPlan(1,ast.limit(),ast.offset()==null?0:ast.offset());
    ...
    private FilterExpressionPlan compileFilter(long root,Expression expression,List<FilterPlan> flat){
        if(expression instanceof AndExpression and)return new FilterExpressionPlan.And(List.of(compileFilter(root,and.getLeftExpression(),flat),compileFilter(root,and.getRightExpression(),flat)));
        if(expression instanceof OrExpression or)return new FilterExpressionPlan.Or(List.of(compileFilter(root,or.getLeftExpression(),flat),compileFilter(root,or.getRightExpression(),flat)));
        FilterPlan predicate=predicate(root,expression); flat.add(predicate); return new FilterExpressionPlan.Predicate(predicate);
    }
    private FilterPlan predicate(long root,Expression expression){
        if(expression instanceof IsNullExpression x)return new FilterPlan(resolve(root,x.getLeftExpression().toString()),x.isNot()?FilterPlan.Operator.IS_NOT_NULL:FilterPlan.Operator.IS_NULL,null);
        if(expression instanceof Between x)return new FilterPlan(resolve(root,x.getLeftExpression().toString()),FilterPlan.Operator.BETWEEN,List.of(value(x.getBetweenExpressionStart()),value(x.getBetweenExpressionEnd())));
        if(expression instanceof InExpression x){String items=x.getRightItemsList()==null?"":x.getRightItemsList().toString();return new FilterPlan(resolve(root,x.getLeftExpression().toString()),FilterPlan.Operator.IN,parseList(items));}
        if(expression instanceof BinaryExpression x)return new FilterPlan(resolve(root,x.getLeftExpression().toString()),operator(x),value(x.getRightExpression()));
