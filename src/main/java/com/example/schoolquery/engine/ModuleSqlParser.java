...
public final class ModuleSqlParser {
    ...
    public ModuleSqlAst parseSelect(String sql) {
        ...
        try {
            ...
            if (ps.getOrderByElements() != null) for (OrderByElement e : ps.getOrderByElements()) sorts.add(new ModuleSqlAst.SortSpec(e.getExpression().toString(), e.isAsc()));
    ...
    private int longValue(Expression expression, String keyword) {
        ...
        return parseNonNegativeInt(Long.toString(v.getValue()), keyword);
    }
    private int parseNonNegativeInt(String value, String keyword) {
        try { long n = Long.parseLong(value); if (n < 0 || n > Integer.MAX_VALUE) throw new IllegalArgumentException(keyword + " is out of range"); return (int) n; }
        catch (NumberFormatException e) { throw new IllegalArgumentException(keyword + " must be a numeric literal", e); }
    }
