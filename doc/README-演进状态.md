...
- `PlanConditionCompiler`：把完整 Boolean Filter Tree 编译成 jOOQ 根 Condition；后代 1:N 字段自动形成相关 EXISTS 链，可同时复用于 DataPlan/CountPlan。
...
2. `PlanConditionCompiler` 已完成统一过滤语义，但尚未替换旧分页服务中的 table-based EXISTS；下一步把它接入新的 Root Data Plan / Count Plan。
...
当前分支已经从“QueryTree 重构”进入“语义计划 + Mutation Pipeline”阶段；剩余工作集中在把旧物理 QueryTree 执行器彻底替换成 QueryPlanExecutor，以及把权限、分页和级联测试闭环。
